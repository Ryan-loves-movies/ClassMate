...
export default function TimetableMain() {
    ...
    useEffect(() => {
        ...
        const updateAddedMod = async (): Promise<modType | undefined> => {
            ...
            if (mods.find((indivMod) => indivMod.code === addedMod) !== undefined) {
                return undefined;
            }
            return await axios.get(`https://api.nusmods.com/v2/2021-2022/modules/${addedMod}.json`, {
                headers: {
                    Accept: 'application/json'
                }
            })
                .then((res: AxiosResponse) => {
                    const modTimetable = ((isSem1)
                        ? (res.data as modInfo).semesterData.filter((lesson) => lesson.semester === 1)
                        : (res.data as modInfo).semesterData.filter((lesson) => lesson.semester === 2)
                    )[0].timetable;
                    const lab = modTimetable.filter((lesson) => lesson.lessonType === "Laboratory")[0];
                    const lecture = modTimetable.filter((lesson) => lesson.lessonType === "Lecture")[0];
                    const tutorial = modTimetable.filter((lesson) => lesson.lessonType !== "Lecture" && lesson.lessonType !== "Laboratory")[0];
                    ...
                    const tempMods = mods.filter((mod) => mod.code !== newMod?.code);
        ...
        const getModules = async () => {
            return await axios.get(`${config.expressHost}/authorized/profile`, {
                ...
                .catch((err: AxiosError) => {
                    alert("Sorry! A problem occured! Your mods could not be found.");
                    console.log(err);
                })
        };
